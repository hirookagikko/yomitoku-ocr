# Extractor リファレンス (v0.11.0 で導入、v0.12.1 で複数値マージ追加)

文書画像/PDFからYAMLスキーマ定義に基づいて構造化データ(JSON)を抽出する機能。

| バージョン | 主な変更 |
|------------|----------|
| v0.11.0 | YomiToku Extractor 初回実装 (`yomitoku_extract` / `yomitoku_extract_with_llm`) |
| v0.12.0 | テキスト認識の回転テキストフォールバック、Searchable-PDF 強化 (Extractor 側は据え置き) |
| v0.12.1 | **Key に対して複数値が存在する場合のマージオプションを追加** (下記「複数値マージ」参照) / Key-Value Parser のアルゴリズム修正 |
| v0.13.0 | 検出/手書き認識モデル強化 (Extractor 側は据え置き、ただし精度の底上げあり) |
...
同一の Key (description / cell_id / bbox) に対して複数の値候補が存在するページ
(例: 「合計金額」が請求書の上部と下部に二度書かれている、テーブルセル分割で同じヘッダーに2つの値が紐づく) で、従来は最初の候補だけが採用されていた。v0.12.1 からは
**フィールドのスキーマ側でマージ挙動を指定できる**:
...
fields:
  - name: total_amount
    ...
    type: number
    normalize: numeric
    merge: first      # 既定。最初の候補を採用
    # merge: concat   # 文字列連結
    # merge: list     # 配列で全候補を返す
```

> オプション名や許容値は CLI 実装側で更新される可能性があるため、最新は
> `yomitoku_extract --help` または [公式ドキュメント](https://kotaro-kinoshita.github.io/yomitoku/) を確認すること。
> このスキルとしては「同じキーに複数値があるなら `merge: list` で取りこぼしを避ける」を推奨ポリシーとする。
